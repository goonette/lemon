			...
			if (base_player) {
				...
				if (auto type_def = base_player->klass->typeMetadataHandle) {
					...
					for (auto index = 0; index < fields_end; index++) {
						...
						auto offset =
							metadata_registration->GetFieldOffsetFromIndex(metadata_handle, base_player->klass, index);

						if (auto type_class = il2cpp::class_from_type(type)) {
							printf("[%i] %s.%s::%s => 0x%X\n", index, type_class->namespaze, type_class->name, name,
								offset);
						}
						else if (auto def = type->get_definition()) {
							auto full_name = def->get_full_name(metadata_handle);
							printf("[%i] %s.%s::%s => 0x%X\n", index, full_name.first, full_name.second, name, offset);
						}
						else {
							printf("[%i] UNK_TYPE.%s => 0x%X\n", index, name, offset);
						}
					}
					...
					for (auto index = 0; index < method_def; index++) {
						auto method_def = metadata_handle->GetMethodDefinitionFromIndex(type_def->methodStart + index);
						...
						for (auto i = 0; i < param_count; i++) {
							auto param =
								metadata_handle->GetParameterDefinitionFromIndex(method_def->parameterStart + i);
						...
						if (auto type_class = il2cpp::class_from_type(ret_type)) {
							printf("[%i] %s.%s::%s(", index, type_class->namespaze, type_class->name, name);
									...
									if (auto param_type_class = il2cpp::class_from_type(param.type)) {
										if (param_count == 1 || i + 1 == param_count) {
											typed_string = std::format("{}{}.{} {}", typed_string,
												param_type_class->namespaze, param_type_class->name, param.name);
										}
										else {
											typed_string = std::format("{}{}.{} {}, ", typed_string,
												param_type_class->namespaze, param_type_class->name, param.name);
										...
										if (param_count == 1 || i + 1 == param_count) {
											typed_string = std::format("{}{}.{} {}", typed_string, full_name.first,
												full_name.second, param.name);
										}
										else {
											typed_string = std::format("{}{}.{} {}, ", typed_string, full_name.first,
												full_name.second, param.name);
										...
										if (param_count == 1 || i + 1 == param_count) {
											typed_string = std::format("{}UNK_TYPE {}", typed_string, param.name);
										}
										else {
											typed_string = std::format("{}UNK_TYPE {}, ", typed_string, param.name);
										}
							...
							// not all methods can get MethodInfo this way for some reason (only overrides ?)
							if (auto method_info = il2cpp::structs::MethodDefinition::
									GetMethodInfoFromMethodDefinitionIndex(type_def->methodStart + index)) {
								auto base_addr = reinterpret_cast<std::uintptr_t>(GetModuleHandleA("GameAssembly.dll"));
								printf("%s) => 0x%llX\n", typed_string.c_str(),
									method_info->get_function_pointer<uintptr_t>() - base_addr);
							}
							else {
								printf("%s)\n", typed_string.c_str());
							}
							...
							printf("[%i] %s.%s::%s(", index, full_name.first, full_name.second, name);
							...
							if (auto method_info = il2cpp::structs::MethodDefinition::
								...
								printf("%s) => 0x%llX\n", typed_string.c_str(),
									method_info->get_function_pointer<uintptr_t>() - base_address);
							}
							else {
								printf("%s)\n", typed_string.c_str());
							}
							...
							printf("[%i] UNK_TYPE::%s(", index, name);
